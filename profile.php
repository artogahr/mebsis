    ...
    <div class="container">
        ...
        if($flag == 0) { // Your Own Profile       
            $postsql = "SELECT posts.post_caption, posts.post_time, users.user_firstname, users.user_lastname,
                                posts.post_public, users.id, users.user_gender, users.user_nickname,
                                users.user_birthdate, users.user_hometown, users.user_status, users.user_about, 
                                posts.post_id
                        FROM posts
                        JOIN users
                        ON users.id = posts.post_by
                        WHERE posts.post_by = $current_id
                        ORDER BY posts.post_time DESC";
            $profilesql = "SELECT users.id, users.user_gender, users.user_hometown, users.user_status, users.user_birthdate,
            ...
            $profilesql = "SELECT users.id, users.user_gender, users.user_hometown, users.user_status, users.user_birthdate,
                            ...
                            FROM users
                            LEFT JOIN (
                                SELECT friendship.user1_id AS id, friendship.friendship_status
                                FROM friendship
                                WHERE friendship.user1_id = $current_id AND friendship.user2_id = {$_SESSION['id']}
                                UNION
                                SELECT friendship.user2_id AS id, friendship.friendship_status
                                FROM friendship
                                WHERE friendship.user1_id = {$_SESSION['id']} AND friendship.user2_id = $current_id
                            ) userfriends
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') { // A form is posted
    if (isset($_POST['request'])) { // Send a Friend Request
        $sql3 = "INSERT INTO friendship(user1_id, user2_id, friendship_status)
                 VALUES ({$_SESSION['id']}, $current_id, 0)";
        ...
        $sql3 = "DELETE FROM friendship
                 WHERE ((friendship.user1_id = $current_id AND friendship.user2_id = {$_SESSION['id']})
                 OR (friendship.user1_id = {$_SESSION['id']} AND friendship.user2_id = $current_id))
        ...
        $sql3 = "INSERT INTO user_phone(id, user_phone) VALUES ({$_SESSION['id']},{$_POST['number']})";
        $query3 = mysqli_query($conn, $sql3);
        if(!$query3){
            echo mysqli_error($conn);
        } 
